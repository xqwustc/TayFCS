...
class BaseModel(nn.Module):
    ...
    def lr_lambda(self, warmup_steps, total_steps):
        def lambda_lr(step):
            ...
            # Cosine Annealing decay schedule: smoothly reduces LR from max to 0 over training steps
            return 0.5 * (1 + torch.cos(torch.tensor(step / total_steps * torch.pi)))
        return lambda_lr
    ...
    def evaluate_with_tayscorer(self, data_generator):
    ...
    def evaluate_with_tayscorer_native(self, data_generator):
        self.eval()
        self.evaluate_with_tayscorer(data_generator)
        return
