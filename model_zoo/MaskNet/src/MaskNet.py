...
### Update for tayfcs start ###
import sys
import os

from model_zoo.DCN.DCN_torch.data_cleaner import convert_csv_to_dataframe
...
class MaskNet(BaseModel):
    def __init__(self,
        ...
        if com_num:
            input_dim += embedding_dim * com_num

        if model_type == "SerialMaskNet":
            self.mask_net = SerialMaskNet(input_dim=input_dim,
                                          output_dim=1,
            ...
            self.mask_net = ParallelMaskNet(input_dim=input_dim,
                                            output_dim=1,
                                            output_activation=self.output_activation,
        ...
        self.num_fields += com_num
        if emb_layernorm:
            self.emb_norm = nn.ModuleList(nn.LayerNorm(embedding_dim) for _ in range(self.num_fields))
        else:
            self.emb_norm = None

        if self.method.endswith('mvfs') and kwargs.get('num_selections', 0) > 0:
            logging.info('--- [MvFS] Use {} feature controllers!'.format(kwargs.get('num_selections', 1)))
            self.controller = Mv.MvFS_Controller(input_dim=get_sum_feature_dimisions(self.embedding_layer),
                                                 embed_dims=len(self.feature_map.features), num_selections=kwargs.get('num_selections', 1))
